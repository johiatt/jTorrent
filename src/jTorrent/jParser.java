...
public class jParser {
	...
	public dList filterTrackers(dList decodedValues) {
		boolean searching = true;
		dList trackers = new dList();

		while (searching) {
			for (DecodedValue dv : decodedValues) {
				if (dv.getContents().endsWith("/announce")) {
					trackers.add(dv);
				}

				if (dv.getContents().equals("comment")) {
					searching = false;
					break;
				}
			}
	...
	public dList parseHashBytes(InputStream stream, int pieces) throws IOException {
		...
		while (stream.available() != 0 && hexCount < (pieces / 20)) {
			...
			// String rest =
			// "&port=6881&uploaded=0&downloaded=0&left=727955456&event=started&numwant=100&no_peer_id=1&compact=1";
